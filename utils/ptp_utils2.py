...
#            if key in attention_maps and len(attention_maps[key]) > 0:  # Check if the key exists and has data
#                for item in attention_maps[key]:
#                    # Reshape the attention map to the expected resolution
#                    print("attn_res:", self.attn_res)  # (96, 120)
#                    print("attn shape:", item.shape) 
#                    cross_maps = item.reshape(-1, self.attn_res[0], self.attn_res[1], item.shape[-1])
#                   out.append(cross_maps)
#        
#        if len(out) == 0:
...
#        return out
    ...
    def aggregate_attention(self, from_where, *, is_cross=True):
        ...
        return running_sum / count        # (H, W, T) 仍可反传梯度
